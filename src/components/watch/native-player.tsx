...
function NativePlayer({ src, showId, mediaType }: NativePlayerProps) {
  ...
  const [embedCompatSrc, setEmbedCompatSrc] = React.useState<string | null>(
    null,
  );
  ...
  React.useEffect(() => {
    ...
    const resolveEmbeddedSource = async () => {
      ...
      try {
        ...
        if (!cancelled) {
          setResolvedSrc(
            `/api/player/hls-proxy?url=${encodeURIComponent(data.streamUrl)}`,
          );
          setResolving(false);
          setEmbedCompatSrc(null);
        }
  ...
  }, [canPlayNatively, resolveAttempt, src]);
  ...
  return (
    <div className="absolute inset-0 bg-black">
      ...
      {resolving ? (
        ...
        <iframe
          className="h-full w-full"
          title="Compatibility provider player"
          src={embedCompatSrc}
          allowFullScreen
        ...
        <div className="absolute inset-0 z-[1] flex items-center justify-center px-4 text-center text-sm text-white/80">
          <div className="flex max-w-md flex-col items-center gap-4">
            ...
            <button
              ...
              className="rounded-md bg-white/90 px-4 py-2 text-xs font-semibold text-black hover:bg-white">
              Retry source
            </button>
          </div>
        </div>
